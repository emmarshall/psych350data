...
All psych350data datasets can be exported as SPSS `.sav` files with full metadata — variable labels, value labels, and user-defined missing values. This is useful when you need to work in SPSS for lab assignments.
...
If you don't provide a path, the file is saved to your current working directory with a default name.
...
Sometimes you only need a few variables from a dataset. You can use `dplyr::select()` to choose specific columns, then pass the result to `export_sav()`:
```{r eval = FALSE}
...
The exported `.sav` file will contain only the selected variables, with all their labels and metadata preserved.
...
Not all datasets use missing values. For example, the `tip_jokes`, `mock_jury`, and `football` datasets are complete — their .sav files have labels but no sentinel missing values.
...
4. If you need to re-export after updating the package, just run the export function again
5. The dataset-specific functions (e.g., `export_superman_sav()`) and the generic `export_sav()` produce identical output
6. **To export a subset of variables**, pipe through `select()` before calling `export_sav()`
